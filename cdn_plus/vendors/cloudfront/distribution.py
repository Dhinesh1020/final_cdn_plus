...
class Cloudfront:
    ...
    def fetch_all_distributions(self):
        ...
        if distribution_response['DistributionList'].get('Items'):
            distribution_list = distribution_response['DistributionList']['Items']
            for each_distribution in distribution_list:
                distribution_config = boto_client.get_distribution_config(Id=each_distribution["Id"])
                each_distribution["State"] = "Enabled" if distribution_config['DistributionConfig']['Enabled'] is True else "Disabled"
    ...
    def fetch_distribution_state_by_domain(self, domain_name=None):
        """
        Gets Distribution State from Edge-location Domain Name.
        Args:
            ** Mandatory **
            domain_name (str) : Cache-server/edge-location domain name
        Returns:
            True (if in enabled state)
            False (if in disabled state)
        ...
        if distributionId_fetch_status:
            ...
            return distribution_config['DistributionConfig']['Enabled']
        else:
            return False   
    ...
    def enable_distribution_by_domain(self, domain_name=None):
        ...
        if distributionId_fetch_status:
            ...
            if distribution_config['DistributionConfig']['Enabled'] == True:
                msg = f"Distribution with domain name {str(domain_name)} is already in enabled state."
                return (False, msg)
            ...
            try:
                ...
                msg = f"The distribution with Domain Name {str(domain_name)} has been enabled."
                return (True, msg)
            except Exception as sdk_error_msg:
                msg = sdk_error_msg
                return (False, msg)
        else:
            msg = "Non-existent Domain received."
            return (False, msg)
    ...
    def disable_distribution_by_domain(self, domain_name=None):
        ...
        if distributionId_fetch_status:
            ...
            if distribution_config['DistributionConfig']['Enabled'] == False:
                msg = f"Distribution with domain name {str(domain_name)} is already in disabled state."
                return (False, msg)            
            ...
            try:
                ...
                msg = f"The distribution with Domain Name {str(domain_name)} has been disabled."
                return (True, msg)
            except Exception as sdk_error_msg:
                msg = sdk_error_msg
                return (False, msg)
    ...
    def delete_distribution_by_domain(self, domain_name=None):
        ...
        if distributionId_fetch_status:
            ...
            if distribution_config['DistributionConfig']['Enabled']:
                msg = f"The distribution with Domain Name {str(domain_name)} is currently deployed and cannot be deleted. Please disable it first."
                return (False, msg) 
            distribution_config = boto_client.get_distribution_config(Id=distribution_id_value)
            try:
                boto_client.delete_distribution(Id=distribution_id_value, IfMatch=distribution_config['ETag'])
                msg = f"The distribution with Domain Name {str(domain_name)} has been deleted."
                return (True, msg)
            except Exception as sdk_error_msg:
                msg = sdk_error_msg
                return (False, msg)
    ...
    def update_distribution_by_domain(self, existing_domain_name=None, **config_payload):
        ...
        if distributionId_fetch_status:
            ...
            try:
                ...
                msg = f"The distribution with Domain Name {str(existing_domain_name)} has been updated."
                return (True, msg)
            except Exception as sdk_error_msg:
                msg = sdk_error_msg
                return (False, msg)
